    ...
    public class SqLiteDb
    {
        ...
        public void CreateTables(IDbModule[] modules)
        {
            foreach (IDbModule module in modules)
            {
                ...
                {
                    columns += ", " + columnToSqlInsert(column);
                }
        ...
        public void AlterAddColumn(IDbModule module, DbColumn column)
        {
            var sql = "ALTER TABLE " + module.TableName + " ADD " + columnToSqlInsert(column);
            ExecuteCommand(sql);
        }

        private string columnToSqlInsert(DbColumn column)
        {
            var sql = column.Name + " " + column.Type;
            if (column.NotNull)
            {
                sql += " NOT NULL";
            }
